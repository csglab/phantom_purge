...
create_grouping_vars <- function(outcomes, S, min_frac) {
...
add_vars_to_outcome_counts <- function(outcomes, S, min_frac=0.8) {
  
  grouping_vars <- create_grouping_vars(outcomes,
                                        S,
                                        min_frac) 
...
infer_sample_of_origin_outcome <- function(..., log_zi, S, p) {
  y <- c(...)[1:S]
  sample_pi <- c(...)[(S + 1):(2 * S)]
...
generate_outcomes <- function(p, S, r, pi_r){
    ...
    outcomes <- add_vars_to_outcome_counts(outcomes, S, min_frac=0.8)
...
r <- low_r:high_r
pdup <- 0.2
F_0 <- pgeom(low_r-1,pdup )
F_1 <- pgeom(high_r,pdup )
norm_fact <- F_1 - F_0
trunc_geom <- dgeom(r, pdup )/norm_fact
p <- 0.90
S <-5
r <-3
alpha <- 1000
